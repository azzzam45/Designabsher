...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Desktop Layout (hidden on desktop) */}
      <div className="hidden lg:block bg-[#F5F5F5]" dir="rtl">
        <DesktopTopNav onNotificationClick={() => setShowNotification(true)} />
        <div className="flex">
          <DesktopSidebar activeMenuItem={activeMenuItem} onMenuItemClick={setActiveMenuItem} />
          {activeMenuItem === 'payments' ? (
            ...
            <div className="flex-1">
              <DriverLicenseRenewal 
                onBack={() => setActiveMenuItem('electronic-services')} 
            ...
            <DesktopMainContent 
              onServiceClick={(service) => {
                if (service === 'driver-license') {
                  setActiveMenuItem('driver-license');
                }
              }}
              walletBalance={walletBalance}
            />
        ...
        <div className="max-w-[430px] mx-auto min-h-screen bg-[#E8F5F1] relative pb-24">
          {activeMobilePage === 'home' ? (
            <>
              ...
              <QuickAccess onServiceClick={(service) => {
                if (service === 'payments') {
                  setActiveMobilePage('payments');
                } else if (service === 'driver-license') {
                  setActiveMobilePage('driver-license');
                }
              }} />
          ...
          ) : activeMobilePage === 'payments' ? (
            <div className="bg-white min-h-screen">
              <GovernmentPayments onBack={() => setActiveMobilePage('home')} />
            </div>
          ) : activeMobilePage === 'driver-license' ? (
            <DriverLicenseRenewal 
              onBack={() => setActiveMobilePage('home')} 
              walletBalance={walletBalance}
              onPaymentComplete={handlePaymentComplete}
            />
          ) : null}
